...
public class InsertSQL
{
	...
	public void execute()
	{
		...
		{	
			...
			// 5. 쿼리 실행 및 결과값 반환
			/*
			 * excuteUpdate() : 쿼리문이 insert/update/delete와 같이 기존레코드에 영향을 미치는 쿼리를 실행할 때 사용하낟.
			 * 					실행 후 영향을 받은 행의 갯수(int)가 반환한다.
			 * 
			 * excuteQuery() : 쿼리문이 select일 때 호출하는 메소드로 레코드에 영향을 미치지 않는 쿼리를 실행한다.
